...
export APP_AGENT_JAVA_OPTS="${JAVA_OPTS} -Dappdynamics.analytics.agent.url=http://machine-agent:9090/v1/sinks/bt ${APPD_CONTROLLER_OPTS} ${APPD_ACCOUNT_OPTS} ${APPD_APPLICATION_OPTS} ${APPD_HOSTID_OPTS} -DjvmRoute=${JVM_ROUTE} -Djava.util.logging.manager=org.apache.juli.ClassLoaderLogManager";
